import { Stock, Category, seller as SellerAPI, Order } from "./api.js";
...
function openDetailPopup(stock) {
  ...
  set('detail-popup-stars', stock.rating ? `<span class="fi fi-ts-star"></span> ${stock.rating}` : '');
...
async function handleConfirmOrder(stock) {
  try {
    ...
    if (!stock.seller_user_id) return alert("Error: Seller information not found");

    await Order.create({
      customer_id: currentUserId,
      seller_id:   stock.seller_user_id,
      items:       [{ stock_id: stock.stock_id, item_quantity: 1 }],
    });
